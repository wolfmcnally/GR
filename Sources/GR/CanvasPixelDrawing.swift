...
extension Canvas {
    ...
    public func drawHorizontalLine(in xRange: Range<Int>, at y: Int, color: Color) {
        drawLine(from: Point(x: Double(xRange.lowerBound), y: Double(y) + 0.5), to: Point(x: Double(xRange.upperBound), y: Double(y) + 0.5), color: color)
    }

    public func drawVerticalLine(in yRange: Range<Int>, at x: Int, color: Color) {
        drawLine(from: Point(x: Double(x) + 0.5, y: Double(yRange.lowerBound)), to: Point(x: Double(x) + 0.5, y: Double(yRange.upperBound)), color: color)
    }
